...
function draw_grid(canvas, ctx, bounds) {
    ...
    for (var i = 1; i < num_vert; i++) {
        ctx.moveTo(width * i - 160, 0);
        ctx.lineTo(width * i - 160, canvas.height);
        ctx.fillText(bounds.days.charAt(i - 1), width * i +2 - 160, 1);
    }
    ctx.closePath();
    ctx.stroke();
    ...
    for (i = 1; i < num_hori; i++) {
        ...
        ctx.lineTo(canvas.width - 160, height * i);
...
function draw_course(canvas, ctx, course, bounds,
    ...
    for (var idx = 0, days = course.days.length; idx < days; idx++) {
        ...
        ctx.fillRect(x - 160, y, width, height);
        ...
        if (text_width >= width - 1) {
            ...
            for (var i = 0, j = words.length; i < j; i++) {
                ctx.fillText(words[i], x + 1 - 160, y + 1 +
                             (bounds.text_height * i));
            }
        }
        else {
            ctx.fillText(course.course, x + 1 - 160, y + 1);
        }
