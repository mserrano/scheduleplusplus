...
function draw_grid(canvas, ctx, bounds) {
    ...
    for (i = 1; i < num_hori; i++) {
        ...
        ctx.fillText(time_to_string(bounds.start + i), text_base, height * i + 1);
        ctx.closePath();
        ctx.stroke();
    }
}
...
function draw_courses(canvas, ctx, courses, bounds) {
    for (var i = 0, j = courses.length; i < j; i++) {
        var blue_color = Math.floor(((200 / j) * courses[i].id) + 100);
        var green_color = Math.floor(50 * (courses[i].id % 3) + 100);
